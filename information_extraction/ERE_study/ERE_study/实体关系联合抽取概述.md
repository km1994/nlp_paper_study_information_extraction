...
- Head entity extraction: 先基于预定义好的Query模型获取指定实体类型下的T个query，与Context拼接后输入并获取T个序列标注结果，记作 $A ={a_1,a_2...a_r}$。并按照下面公式做结果集成。其中 $f_t$ 是第t个query下，开发集上计算出的序列标注F1 score。解码 $a_i^+$ 获取实体结果。
